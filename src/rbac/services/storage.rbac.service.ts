...
export class StorageRbacService {
  ...
  async getPermissions(): Promise<{ [key: string]: any }> {
    return (await this.rbac.getRbac()).permissions;
  }

  async getGrants(): Promise<{ [key: string]: any }> {
  ...
  async getGrant(role: string): Promise<string[]> {
    const grant: { [key: string]: any } = await this.parseGrants();

    return grant[role] || [];
  }

  async getFilters(): Promise<{ [key: string]: any }> {
    const result: any = {};
  ...
  private async parseGrants(): Promise<{ [key: string]: any }> {
    if (this.cache) {
  ...
  private async getFromCache(): Promise<{ [key: string]: any } | null> {
    return this.cache.get();
  }

  private async setIntoCache(value: { [key: string]: any }): Promise<void> {
